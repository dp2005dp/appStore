...
class AppStore extends Component {
  state = {tapItem: tabsList[0].tabId, searchAppItam: ''}

  searchApp = event => {
    this.setState(() => ({searchAppItam: event.target.value}))
  }

  filterAppList = () => {
    const {tapItem, searchAppItam} = this.state
    const filterApps = appsList.filter(eachApp => eachApp.category === tapItem)
    const searchApps = filterApps.filter(eachAppsearch =>
      eachAppsearch.appName
        .toLowerCase()
        .includes(searchAppItam.toLocaleLowerCase()),
    )
    return searchApps
  }

  onTapItam = tabId => {
    this.setState(() => ({tapItem: tabId}))
  }
  ...
  renderAppsView = () => {
    const searchApps = this.filterAppList()
    return (
      <ul className="app-itam-container">
        {searchApps.map(eachApplist => (
          <AppItem appItamList={eachApplist} key={eachApplist.appId} />
        ))}
      </ul>
    )
  }
      ...
      <div className="app-container">
        ...
        <ul className="tap-iteam-container">
          {tabsList.map(eachTapItem => (
            <TabItem
              tabItemList={eachTapItem}
              key={eachTapItem.tabId}
              onTapItam={this.onTapItam}
              activeTapItem={tapItem === eachTapItem.tabId}
            />
          ))}
        </ul>
